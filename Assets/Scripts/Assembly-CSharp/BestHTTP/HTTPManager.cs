...
namespace BestHTTP
{
	public static class HTTPManager
	{
		...
		private static string GetKeyForRequest(HTTPRequest request)
		{
			...
			return new UriBuilder(request.CurrentUri.Scheme, request.CurrentUri.Host, request.CurrentUri.Port).Uri.ToString();
		}
			...
			if (Connections.TryGetValue(keyForRequest, out value))
			{
				...
				if (num <= MaxConnectionPerServer)
				{
					for (int j = 0; j < value.Count; j++)
					{
						...
						if (connectionBase2 != null && connectionBase2.IsFree && (connectionBase2.LastProcessedUri == null || connectionBase2.LastProcessedUri.Host.Equals(request.CurrentUri.Host, StringComparison.OrdinalIgnoreCase)))
						{
