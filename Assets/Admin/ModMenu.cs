...
public class ModMenu : MonoBehaviour
{
    ...
    public static bool toggle9;

    public static bool toggle11;

    public static bool toggle12;
    ...
    public static bool enableClanLogging;
    ...
    public static void EnableClanLogging()
    {
        enableClanLogging = true;
    }
    ...
    public void ModMenuGUI()
    {
        ...
        //GUI.Label(new Rect(ModMenu.MenuRect.x + 55f, ModMenu.MenuRect.y + 85f, ModMenu.widthSize - 86f, 95f), "   Mod by BlockStrayker", ModMenu.LabelStyle);
...
12,
            ...
            if (GUI.Button(ModMenu.BtnRect3(16), "Kill", ModMenu.BtnStyle))
            {
                RPC(new object[]
                {
                    "2h6gm88xz",
12,
                    "local damage = CS.DamageInfo();damage.AttackerTeam = CS.Team.None;damage.Damage = 10000;damage.PlayerID = -1;damage.WeaponID = 0;damage.AttackPosition = CS.Vector3.zero;CS.PlayerInput.instance:Damage(damage);",
                    true
                });
                //Log("Kill");
            }
            if (GUI.Button(ModMenu.BtnRect3(17), "Give 888888 Gold", ModMenu.BtnStyle))
            {
                RPC(new object[]
                {
                    "2h6gm88xz",
12,
                    "CS.AccountManager.SetGold(888888);CS.AccountManager.UpdateGold(888888, null, null);",
                    true
                });
                //Log("Kill");
            }
            if (GUI.Button(ModMenu.BtnRect3(18), "Give 888888 Money", ModMenu.BtnStyle))
            {
                RPC(new object[]
                {
                    "2h6gm88xz",
12,
                    "CS.AccountManager.SetMoney(888888);CS.AccountManager.UpdateMoney(888888, null, null);",
                    true
                });
                //Log("Kill");
            }
...
12,
            ...
            if (GUI.Button(ModMenu.BtnRect3(20), "Lua Me", ModMenu.BtnStyle))
            {
                if (mAccount.isAdmin)
                {
                    ...
                    luaEnv.DoString(ModMenu.Text, "chunk", null);
                    luaEnv.Dispose();
                }
                else
                {
                    UIToast.Show("Error: Only ban available");
                }
            }
            GUI.EndScrollView();
        }
        if (ModMenu.PageID == 2)
        {
            ...
            if (ModMenu.toggle11)
            {
                if (GUI.Button(ModMenu.BtnRect(3, false), "FriendDamage: ON", ModMenu.BtnStyle))
                {
                    if (mAccount.isAdmin)
                    {
                        ModMenu.toggle11 = false;
                       // GameManager.SetFriendDamage(false);
                    }
                    else
                    {
                        UIToast.Show("Error: Only ban available");
        ...
        if (ModMenu.PageID == 3)
        {
            ...
            if (eventTarget == EventTargets.Player && selectedPlayer != null)
            {
                if (GUI.Button(ModMenu.BtnRect3(5), "Current Target: " + selectedPlayer.NickName, ModMenu.BtnStyle))
                {
            ...
            if (GUI.Button(ModMenu.BtnRect3(6), "BlueSpawn", ModMenu.BtnStyle))
            {
                RPC(new object[]
                {
                    "2h6gm88xz",
12,
                    "CS.UnityEngine.GameObject.Find('Player').transform.position = CS.UnityEngine.GameObject.Find('Spawns').transform:Find('Default').transform:Find('Blue').transform.position;",
                    true
                });
                //Log("BlueSpawn");
            }
            if (GUI.Button(ModMenu.BtnRect3(7), "RedSpawn", ModMenu.BtnStyle))
            {
                RPC(new object[]
                {
                    "2h6gm88xz",
12,
                    "CS.UnityEngine.GameObject.Find('Player').transform.position = CS.UnityEngine.GameObject.Find('Spawns').transform:Find('Default').transform:Find('Red').transform.position;",
                    true
                });
                //Log("RedSpawn");
            }
...
12,
        ...
        if (ModMenu.PageID == 6 && GUI.Button(ModMenu.BtnRect(3, false), "Maps Menu", ModMenu.BtnStyle))
        {
            ModMenu.PageID = 0;
            ModMenu.SettingsOpen = false;
            ...
            for (int j = 0; j < GameSettings.instance.Weapons.Count; j++)
            {
                if (GUI.Button(ModMenu.BtnRect3(j + 1), GameSettings.instance.Weapons[j].Name, ModMenu.BtnStyle))
                {
                    RPC(new object[]
                    {
                        "2h6gm88xz",
12,
                        "CS.WeaponManager.SetSelectWeapon(CS.WeaponType." + GetTypeFromName(GameSettings.instance.Weapons[j].Name) + ", " + GetIDFromName(GameSettings.instance.Weapons[j].Name) + ");CS.GameManager.instance.Controller.PlayerInput.PlayerWeapon:UpdateWeaponAll(CS.WeaponType." + GetTypeFromName(GameSettings.instance.Weapons[j].Name) + ");",
                        true
                    });
        ...
        if (ModMenu.PlayersListOpen && ModMenu.PageID == 0)
        {
            ...
            for (int i = 0; i < PhotonNetwork.otherPlayers.Length; i++)
            {
                ...
                {
                    if (GUI.Button(ModMenu.BtnRect3(i + 1), PhotonNetwork.otherPlayers[i].NickName, ModMenu.BtnStyle))
                    {
                        eventTarget = EventTargets.Player;
                ...
                {
                    if (GUI.Button(ModMenu.BtnRect(i + 1, false), PhotonNetwork.otherPlayers[i].NickName, ModMenu.BtnStyle))
                    {
                        eventTarget = EventTargets.Player;
                        selectedPlayer = PhotonNetwork.otherPlayers[i];
                        ModMenu.PageID = 3;
            ...
            if (ModMenu.mapsGameMode == GameMode.BuildBattle)
            {
                if (GUI.Button(ModMenu.BtnRect(1, false), "Logo", ModMenu.BtnStyle))
                {
                    ...
                    RPC(new object[]
                    {
                        "2h6gm88xz",
12,
                        "CS.UnityEngine.Application.LoadLevel(" + "'Logo'" + ");",
                        true
                    });
                }
                if (GUI.Button(ModMenu.BtnRect(2, false), "MainTutorial", ModMenu.BtnStyle))
                {
                    ...
                    RPC(new object[]
                    {
                        "2h6gm88xz",
12,
                        "CS.UnityEngine.Application.LoadLevel(" + "'MainTutorial'" + ");",
                        true
                    });
                }
                if (GUI.Button(ModMenu.BtnRect(3, false), "Shooting Range", ModMenu.BtnStyle))
                {
                    ...
                    RPC(new object[]
                    {
                        "2h6gm88xz",
12,
                        "CS.UnityEngine.Application.LoadLevel(" + "'Shooting Range'" + ");",
                        true
                    });
                }
                return;
            ...
            for (int k = 0; k < LevelManager.GetGameModeScenes(ModMenu.mapsGameMode).Count; k++)
            {
                if (GUI.Button(ModMenu.BtnRect3(k + 1), LevelManager.GetGameModeScenes(ModMenu.mapsGameMode)[k], ModMenu.BtnStyle))
                {
                    ...
                    RPC(new object[]
                    {
                        "2h6gm88xz",
12,
                        "CS.UnityEngine.Application.LoadLevel('" + LevelManager.GetGameModeScenes(ModMenu.mapsGameMode)[k] + "');",
                        true
                    });
                }
            }
        ...
        if (ModMenu.GameModesOpen && ModMenu.PageID == 0)
        {
            ...
            for (int l = 0; l < 20; l++)
            {
                ...
                if (GUI.Button(ModMenu.BtnRect3(20), "Others", ModMenu.BtnStyle))
                {
